...
pub fn handler<'key, 'accounts, 'remaining, 'info>(ctx: Context<'key, 'accounts, 'remaining, 'info, InvalidateCtx<'info>>) -> Result<()> {
    ...
    match (
        ...
        (TokenManagerState::Claimed, TokenManagerKind::Managed) => thaw_non_edition(mutable_ctx, mutable_ctx.program_id, remaining_accs)?,
        (TokenManagerState::Claimed, TokenManagerKind::Permissioned) => thaw_non_edition(mutable_ctx, mutable_ctx.program_id, remaining_accs)?,
...
pub fn thaw_non_edition<'key, 'accounts, 'remaining, 'info>(
        ...
        Ok(_) => {
            ...
            match metadata.token_standard {
                Some(TokenStandard::ProgrammableNonFungible) => {
                    ctx.accounts.token_manager.kind = TokenManagerKind::Programmable as u8;
                }
