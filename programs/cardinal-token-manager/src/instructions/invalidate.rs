...
pub fn handler<'key, 'accounts, 'remaining, 'info>(ctx: Context<'key, 'accounts, 'remaining, 'info, InvalidateCtx<'info>>) -> Result<()> {
    ...
    match (
        ...
        // Managed
        (TokenManagerKind::Managed, InvalidationType::Return) => ctx.accounts.return_invalidation(remaining_accs)?,
        (TokenManagerKind::Managed, InvalidationType::Invalidate) => ctx.accounts.invalidate_invalidation()?,
        (TokenManagerKind::Managed, InvalidationType::Release) => ctx.accounts.release_invalidation()?,
        (TokenManagerKind::Managed, InvalidationType::Reissue) => ctx.accounts.reissue_invalidation()?,
        (TokenManagerKind::Managed, InvalidationType::Vest) => ctx.accounts.vest_invalidation(remaining_accs)?,
        // Editions
        (TokenManagerKind::Edition, InvalidationType::Return) => ctx.accounts.return_invalidation(remaining_accs)?,
        (TokenManagerKind::Edition, InvalidationType::Invalidate) => ctx.accounts.invalidate_invalidation()?,
        (TokenManagerKind::Edition, InvalidationType::Release) => ctx.accounts.release_invalidation()?,
        (TokenManagerKind::Edition, InvalidationType::Reissue) => ctx.accounts.reissue_invalidation()?,
        (TokenManagerKind::Edition, InvalidationType::Vest) => ctx.accounts.vest_invalidation(remaining_accs)?,
        // Programmable
        (TokenManagerKind::Programmable, InvalidationType::Return) => ctx.accounts.return_pnft_invalidation(remaining_accs)?,
        (TokenManagerKind::Programmable, InvalidationType::Release) => ctx.accounts.release_pnft_invalidation(remaining_accs)?,
        ...
        // Permissioned
        (TokenManagerKind::Permissioned, InvalidationType::Return) => ctx.accounts.return_invalidation(remaining_accs)?,
        (TokenManagerKind::Permissioned, InvalidationType::Invalidate) => ctx.accounts.invalidate_invalidation()?,
        (TokenManagerKind::Permissioned, InvalidationType::Release) => ctx.accounts.release_invalidation()?,
        (TokenManagerKind::Permissioned, InvalidationType::Reissue) => ctx.accounts.reissue_invalidation()?,
        (TokenManagerKind::Permissioned, InvalidationType::Vest) => ctx.accounts.vest_invalidation(remaining_accs)?,
        // Unmanaged
        (TokenManagerKind::Unmanaged, InvalidationType::Return) => ctx.accounts.return_invalidation(remaining_accs)?,
        (TokenManagerKind::Unmanaged, InvalidationType::Invalidate) => ctx.accounts.invalidate_invalidation()?,
        (TokenManagerKind::Unmanaged, InvalidationType::Release) => ctx.accounts.release_invalidation()?,
        (TokenManagerKind::Unmanaged, InvalidationType::Reissue) => ctx.accounts.reissue_invalidation()?,
        (TokenManagerKind::Unmanaged, InvalidationType::Vest) => ctx.accounts.vest_invalidation(remaining_accs)?,
...
impl<'info> InvalidateCtx<'info> {
    pub fn thaw_non_edition(self, program_id: &Pubkey, remaining_accs: &mut Iter<AccountInfo<'info>>) -> Result<()> {
    ...
    pub fn thaw_edition(self, remaining_accs: &mut Iter<AccountInfo<'info>>) -> Result<()> {
        // get PDA seeds to sign with
        let token_manager_seeds = &[TOKEN_MANAGER_SEED.as_bytes(), self.token_manager.mint.as_ref(), &[self.token_manager.bump]];
        ...
        match assert_derivation(
            ...
            &[mpl_token_metadata::state::PREFIX.as_bytes(), mpl_token_metadata::id().as_ref(), self.mint.key().as_ref()],
        ) {
            // migrated pnft
            Ok(_) => {
                ...
                match metadata.token_standard {
                    Some(TokenStandard::ProgrammableNonFungible) => {
                        self.token_manager.kind = TokenManagerKind::Programmable as u8;
                    }
            ...
            _ => {
                ...
                invoke_signed(
                    &thaw_delegated_account(*metadata_program.key, self.token_manager.key(), self.recipient_token_account.key(), *edition_info.key, self.mint.key()),
                    &[
                        self.token_manager.to_account_info(),
    ...
    pub fn reissue_invalidation(mut self) -> Result<()> {
        // get PDA seeds to sign with
        let token_manager_seeds = &[TOKEN_MANAGER_SEED.as_bytes(), self.token_manager.mint.as_ref(), &[self.token_manager.bump]];
        let token_manager_signer = &[&token_manager_seeds[..]];
        ...
        let cpi_accounts = Transfer {
            ...
            to: self.token_manager_token_account.to_account_info(),
            authority: self.token_manager.to_account_info(),
        };
        let cpi_program = self.token_program.to_account_info();
        let cpi_context = CpiContext::new(cpi_program, cpi_accounts).with_signer(token_manager_signer);
        ...
        self.token_manager.state = TokenManagerState::Issued as u8;
        self.token_manager.recipient_token_account = self.token_manager_token_account.key();
        self.token_manager.state_changed_at = Clock::get().unwrap().unix_timestamp;

        let required_lamports = self.rent.minimum_balance(self.token_manager.to_account_info().data_len());
        let token_manager_lamports = self.token_manager.to_account_info().lamports();
        if token_manager_lamports > required_lamports {
            ...
            **self.token_manager.to_account_info().try_borrow_mut_lamports()? = required_lamports;
            **self.collector.to_account_info().try_borrow_mut_lamports()? = self.collector.to_account_info().lamports().checked_add(diff).expect("Add error");
        };

        Ok(())
    }

    pub fn release_pnft_invalidation(self, remaining_accs: &mut Iter<AccountInfo<'info>>) -> Result<()> {
        // get PDA seeds to sign with
        let token_manager_seeds = &[TOKEN_MANAGER_SEED.as_bytes(), self.token_manager.mint.as_ref(), &[self.token_manager.bump]];
        let token_manager_signer = &[&token_manager_seeds[..]];
        ...
        if self.token_manager.receipt_mint.is_none() {
            if return_token_account.owner != self.token_manager.issuer {
            ...
            if !(receipt_token_account.mint == self.token_manager.receipt_mint.expect("No receipt mint") && receipt_token_account.amount > 0) {
        ...
        invoke_signed(
            &Instruction {
                ...
                accounts: vec![
                    // 0. `[signer]` Delegate
                    AccountMeta::new_readonly(self.token_manager.key(), true),
        ...
        invoke_signed(
            &Instruction {
                ...
                accounts: vec![
                    ...
                    // 1. `[]` Delegated owner
                    AccountMeta::new_readonly(self.token_manager.key(), false),
                ...
                data: MetadataInstruction::Revoke(RevokeArgs::LockedTransferV1).try_to_vec().unwrap(),
            },
            &[
                self.token_manager.to_account_info(),
                mint_metadata_info.to_account_info(),
        ...
        let cpi_accounts = CloseAccount {
            account: self.token_manager_token_account.to_account_info(),
            destination: self.collector.to_account_info(),
            authority: self.token_manager.to_account_info(),
        };
        let cpi_program = self.token_program.to_account_info();
        let cpi_context = CpiContext::new(cpi_program, cpi_accounts).with_signer(token_manager_signer);
        ...
        // close token_manager
        self.token_manager.state = TokenManagerState::Invalidated as u8;
        self.token_manager.state_changed_at = Clock::get().unwrap().unix_timestamp;
        self.token_manager.close(self.collector.to_account_info())?;

        Ok(())
    }

    pub fn return_pnft_invalidation(self, remaining_accs: &mut Iter<AccountInfo<'info>>) -> Result<()> {
        // get PDA seeds to sign with
        let token_manager_seeds = &[TOKEN_MANAGER_SEED.as_bytes(), self.token_manager.mint.as_ref(), &[self.token_manager.bump]];
        let token_manager_signer = &[&token_manager_seeds[..]];
        ...
        invoke_signed(
            &Instruction {
                ...
                accounts: vec![
                    ...
                    AccountMeta::new_readonly(self.recipient_token_account.owner.key(), false),
                    // #[account(2, writable, name="destination", desc="Destination token account")]
                    AccountMeta::new(self.token_manager_token_account.key(), false),
                ...
                data: MetadataInstruction::Transfer(TransferArgs::V1 {
                    amount: self.token_manager.amount,
            ...
            &[
                self.recipient_token_account.to_account_info(),
                recipient_token_account_owner_info.to_account_info(),
                self.token_manager_token_account.to_account_info(),
                self.token_manager.to_account_info(),
                mint_info.to_account_info(),
                mint_metadata_info.to_account_info(),
                mint_edition_info.to_account_info(),
    ...
    pub fn release_invalidation(self) -> Result<()> {
        // get PDA seeds to sign with
        let token_manager_seeds = &[TOKEN_MANAGER_SEED.as_bytes(), self.token_manager.mint.as_ref(), &[self.token_manager.bump]];
        let token_manager_signer = &[&token_manager_seeds[..]];
    ...
    pub fn invalidate_invalidation(self) -> Result<()> {
        // get PDA seeds to sign with
        let token_manager_seeds = &[TOKEN_MANAGER_SEED.as_bytes(), self.token_manager.mint.as_ref(), &[self.token_manager.bump]];
        let token_manager_signer = &[&token_manager_seeds[..]];
    ...
    pub fn return_invalidation(self, remaining_accs: &mut Iter<AccountInfo<'info>>) -> Result<()> {
        // get PDA seeds to sign with
        let token_manager_seeds = &[TOKEN_MANAGER_SEED.as_bytes(), self.token_manager.mint.as_ref(), &[self.token_manager.bump]];
        let token_manager_signer = &[&token_manager_seeds[..]];
    ...
    pub fn vest_invalidation(self, remaining_accs: &mut Iter<AccountInfo<'info>>) -> Result<()> {
        // get PDA seeds to sign with
        let token_manager_seeds = &[TOKEN_MANAGER_SEED.as_bytes(), self.token_manager.mint.as_ref(), &[self.token_manager.bump]];
        let token_manager_signer = &[&token_manager_seeds[..]];

        if self.token_manager.state == TokenManagerState::Issued as u8 {
            // find claim_approver token account
            let claim_approver_token_account_info = next_account_info(remaining_accs)?;
            let claim_approver_token_account = Account::<TokenAccount>::try_from(claim_approver_token_account_info)?;
            if claim_approver_token_account.owner != self.token_manager.claim_approver.expect("No claim approver found") {
