...
pub struct CloseCtx<'info> {
    ...
    #[account(constraint = token_manager.key() == transfer_authority.token_manager @ ErrorCode::InvalidTokenManager)]
    token_manager: UncheckedAccount<'info>,

    #[account(mut)]
    transfer_authority: Box<Account<'info, TranssferAuthority>>,

    #[account(mut, constraint = collector.key() == transfer_authority.collector @ ErrorCode::InvalidCollector)]
...
pub fn handler(ctx: Context<CloseCtx>) -> Result<()> {
    ...
    //     ctx.accounts.transfer_authority.close(ctx.accounts.collector.to_account_info())?;
    // } else {
